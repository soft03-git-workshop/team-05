...
| 役割 | 作業 |
|---|---|
| **担当A：講座名** | [講座名と説明を追加する](https://github.com/soft03-git-workshop/team-05/issues/16)。先にPRをマージする。 |
| **担当B：team名** | [team名と紹介を追加する](https://github.com/soft03-git-workshop/team-05/issues/17)。その後、rebaseで解消する。 |
...
ブランチが `feature/17` で、変更が残っておらず、すべてpush済みであることを確認します。

```bash
git fetch origin
git rev-parse HEAD
git rev-parse origin/feature/17
```
...
ブランチが `feature/17` であることと差分を確認し、新旧のcommitのIDを見比べます。rebaseでcommitを作り直すため、IDが変わります。

次のコマンドは、GitHub上のブランチが手順4で控えたIDのままである場合だけ、**Bの作業ブランチ**を更新します。

```bash
git push --force-with-lease="refs/heads/feature/17:$workshop_before_rebase" origin HEAD:refs/heads/feature/17
```
